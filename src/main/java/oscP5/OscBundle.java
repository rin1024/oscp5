...
public class OscBundle extends OscPacket {
   ...
   * events at a given time in the future or immediately. timetags can only be set for osc bundles,
   * not for osc messages. oscP5 supports receiving timetags, but does not queue messages for
   * execution at a set time.
   */
  ...
  public final String toString() {
    return String.format(
        "OscBundle{hostAddress=%s, port=%s, messages=%s}", hostAddress, port, size());
  }
}
