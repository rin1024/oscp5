...
public class OscMessage extends OscPacket {
  ...
  /**
   * returns an OscArgument from which the value can be parsed into the right format. e.g. to parse
   * an int from the first argument in the osc message, use theOscMessage.get(0).intValue();
   *
   * @param theIndex int
   * @return OscArgument
   */
  ...
  public final String toString() {
    return String.format(
        "OscMessage{hostAddress=%s, port=%s, address=%s, typetag=%s, data-size=%s, data=%s}",
        hostAddress,
        port,
        getAddress(),
        getTypetag(),
        getArguments().length,
        arrayToString(getArguments()));
  }
  ...
  public void indexOutOfBounds(int n) {
    System.err.println(
        "ArrayIndexOutOfBounds: index requested " + n + ", expected < " + getArguments().length);
  }
